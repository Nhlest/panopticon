...
use bevy::render::render_resource::{BindGroupDescriptor, BindGroupEntry, BindingResource, BufferInitDescriptor, BufferUsages};
...
pub fn queue_bind_group(
  ...
  for (i, j) in query.iter() {
    spheres.push(Sphere::new(j.color.as_rgba_f32(), [i.translation.x, i.translation.y, i.translation.z], 0.5));
  }
...
pub fn extract_spheres(
  ...
  for (i, j) in spheres.iter() {
    commands.spawn((i.clone(), ColorComponent { color : materials.get(j).unwrap().base_color }, SphereTag));
  }
}
